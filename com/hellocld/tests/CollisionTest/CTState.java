...
public class CTState extends GameState {
	...
	public void create() {
		super.create();
		//make a CollisionGroup handler entity
		int collisionGroups = em.createEntity();
		em.addComponent(collisionGroups, new CollisionGroup());
		//create a group for all the bouncingBoxes created later
		em.getComponent(collisionGroups, CollisionGroup.class).createGroup("bouncingBoxes");
		...
		for(int i = 0; i<4; i++) {
			...
			em.getComponent(walls[i], Collision2D.class).checkGroups.add("bouncingBoxes");
		}
		...
		em.getComponent(walls[0], Collision2D.class).setSize(319, 1);
		...
		em.getComponent(walls[1], Collision2D.class).setSize(1, 239);
		...
		em.getComponent(walls[2], Collision2D.class).setSize(319, 1);
		...
		em.getComponent(walls[3], Collision2D.class).setSize(1, 239);
		
		systems.add(new Render2DSystem());
		systems.add(new SimpleMoveSystem());
		systems.add(new BouncingBoxGenerator(collisionGroups));
		systems.add(new Collision2DSystem(collisionGroups));
